...
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `ffmpeg` + `ffprobe`           | `VideoOptimiser.cs` performs remuxes, re-encodes, animated exports, and metadata probes; `FfprobeMetadataProbe` shells out to `ffprobe` for stream details. | Stage the BtbN `ffmpeg-master-latest-win64-gpl.zip` payload under `tools/ffmpeg/` so the AMF/NVENC-enabled binaries copy into `%LOCALAPPDATA%\Clop\bin\{arch}` at runtime. | GPL/LGPL depending on codecs. Bundle the upstream GPL notices and document the source URL inside THIRD_PARTY_NOTICES.               |
...
| `LibreOffice` (`soffice.com`)  | `DocumentOptimiser` converts DOC/DOCX/XLS/PPT/etc. to PDF before handing them to `PdfOptimiser`.                                                            | Download the official MSI via `scripts/fetch-tools.ps1 -Name libreoffice`, extract into `tools/libreoffice/`, and keep the `program/` + `share/` folders intact.           | Dual-licensed MPL 2.0 / LGPL v3+. Ship both license texts and keep upstream `license.txt` with the binary payload.                  |
